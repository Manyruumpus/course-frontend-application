...
const Matches=() =>{
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-slate-100 to-blue-100 p-10">
      <h1 className="text-4xl font-extrabold text-slate-700 mb-10 text-center">
        Meet Your Matches
      </h1>
        ...
        {profiles.map(({ id, name, avatar, Batch, Branch ,Gender}) => (
          ...
          >
            <img
              ...
              loading="lazy"
            />
            <h2 className="text-xl font-semibold text-slate-800 mb-1">{name}</h2>
            <p className="text-slate-600 font-medium ">{Batch}</p>
            <p className="text-slate-500 text-sm leading-relaxed">{Branch}</p>
            <p className="text-slate-500 text-sm leading-relaxed">{Gender}</p>
          </div>
        ))}
      </div>
    </div>
