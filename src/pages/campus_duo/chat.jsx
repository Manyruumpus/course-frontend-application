...
export default function Chat() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] select-none bg-gradient-to-b from-slate-100 to-slate-400 font-sans">
      {/* Sidebar */}
      <aside className="w-72 bg-white shadow-lg flex flex-col">
        <div className="px-6 py-5 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-slate-600 to-slate-600 text-white font-semibold tracking-wide text-lg">
          Chats
          <button
            title="New Chat"
            className="p-1 rounded-full hover:bg-slate-400 transition-colors"
        ...
        <div className="overflow-y-auto flex-grow">
          {contacts.map(contact => (
            <button
              ...
              className={`flex items-center gap-3 w-full px-5 py-4 cursor-pointer hover:bg-slate-50 transition-colors ${
                currentContact === contact.id ? "bg-slate-100" : ""
              }`}
            ...
            >
              ...
              <div className="flex flex-col items-start">
                <span className="font-semibold text-slate-400">{contact.name}</span>
      ...
      {/* Chat Window */}
      <main className="flex flex-col flex-grow max-h-full justify-between bg-slate-50 shadow-inner">
        {/* Header */}
        <header className="flex items-center px-6 py-4 bg-gradient-to-r from-slate-600 to-purple-600 border-b border-slate-700 text-white shadow-md">
          <img
          ...
          <div className="ml-auto flex gap-3">
            <button aria-label="Video Call" className="hover:bg-slate-700 p-2 rounded-full transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path d="M23 7l-7 5 7 5V7z" />
                <rect x="1" y="5" width="15" height="14" rx="2" ry="2" />
              </svg>
            </button>
            <button aria-label="Call" className="hover:bg-slate-700 p-2 rounded-full transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <button aria-label="More options" className="hover:bg-slate-700 p-2 rounded-full transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            >
              <div
                className={`relative px-5 py-3 rounded-xl shadow-md break-words whitespace-pre-wrap select-text transition-all duration-300 ${
                  sender === "me"
                    ? "bg-gradient-to-br from-purple-500 to-slate-600 text-white rounded-br-none animate-fadeInRight"
                    : "bg-slate-100 text-slate-900 rounded-bl-none animate-fadeInLeft"
                }`}
                style={{ wordBreak: "break-word" }}
              >
                {text}
              </div>
              <span className="mt-1 text-xs text-slate-400 font-mono select-none">
                {time}
              </span>
            </div>
          ))}
        ...
        {/* Input Area */}
        <footer className="px-6 py-4 bg-gradient-to-r from-slate-50 to-purple-50 border-t border-slate-300 flex items-center gap-3">
          <button aria-label="Add attachment" className="text-slate-500 hover:text-slate-700 p-2 rounded-full transition-colors" type="button">
          ...
          <textarea
            ...
            className="flex-grow resize-none bg-white shadow-sm rounded-full px-5 py-3 text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-1 transition-all max-h-24 overflow-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100"
          />
          <button
            onClick={sendMessage}
            disabled={!input.trim()}
            title="Send Message"
            className="p-3 rounded-full bg-slate-600 flex items-center justify-center transition-colors hover:bg-slate-700 shadow-lg text-white disabled:opacity-50 disabled:cursor-not-allowed"
